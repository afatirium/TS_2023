...
Additionally, I want to mention that, I am inspired from two article for this project:
...
#Estimating
portfolio_IS.ar1egarchmt11 <- ugarchfit(spec = spec, data = portfolio_IS$PORTFOLIO_r)
```

```{r}
# Plot
plot(portfolio_IS.ar1egarchmt11, which = 10)
plot(portfolio_IS.ar1egarchmt11, which = 11)
```

**Fitting model**

```{r}
head(portfolio_IS.ar1egarchmt11@fit$sigma)
```
...
portfolio_IS$VaR <- q01 * portfolio_IS.ar1egarchmt11@fit$sigma
tail(portfolio_IS)
head(portfolio_IS)
```
...
abline(h = 0, lty = 2)
lines(portfolio_IS$VaR, type = 'l', col = "green")
```

## VaR Out-of-sample period

Choosing VaR in the Oot-of-Sample(OS) period:
